...
class zabbix_api: 
	...
	def host_create(self,host,name, hostip, hostgroupName, templateName,proxy_id):
            zab_result=""
            if self.host_get(hostip):
                zab_result="\033[041m该主机已经添加!\033[0m"
                sys.exit(1)
            ...
            try:
                ...
                response = json.loads(result.read())
                result.close()
                zab_result="添加主机 : \033[42m%s\031[0m \tid :\033[31m%s\033[0m"%(hostip, response['result']['hostids'])
            return zab_result
